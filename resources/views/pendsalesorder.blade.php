...
?>
	...
	<div class="container-fluid">
		<div class="title-heading" style="text-align: center;">
			Requesting Sales Order<hr style="background-color: black">
		</div>
			...
			@if(count($data)>0)
				@foreach($data as $row)
					<tr>
						...
						<td>
							<form action="{{ route('processsalesorder') }}" method="post">
								{{ csrf_field() }}
								<input type="text" name="id" style="display: none" value="{{ $row->Sales_order_id }}">
								<button class="btn btn-primary">Continue Processing</button>
							</form>
								
						</td>
						<td>
							<form action="{{ route('pendingorderview') }}" method="post">
								{{csrf_field()}}
								<input type="text" name="id" style="display: none" value="{{ $row->Sales_order_id }}">
