...
?>
	...
	<div class="container-fluid">
		<div class="title-heading" style="text-align: center;">
			Processing Sales Order<hr style="background-color: black">
		</div>
			...
			@if(count($data)>0)
				@foreach($data as $row)
					<tr>
						...
						<td>
							<form action="{{ route('complateorder') }}" method="post">
								{{ csrf_field() }}
								<input type="text" name="id" style="display: none;" value="{{ $row->Sales_order_id }}">
								<button class="btn btn-primary" style="width: 100%">Complate order</button>
							</form>
								
						</td>
						<td>
							<form action="processorderview" method="post">
								{{csrf_field()}}
								<input type="text" name="id" style="display: none" value="{{ $row->Sales_order_id }}">
